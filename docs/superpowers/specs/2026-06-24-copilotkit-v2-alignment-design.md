...
([CopilotKit/examples/integrations/langgraph-python](https://github.com/CopilotKit/CopilotKit/tree/main/examples/integrations/langgraph-python))
...
1. **The real app is a thin CopilotKit v1 wrapper.** `ui/app/` uses `@copilotkit/* ^1.10.5`
   ...
   `createCopilotEndpoint` over Hono, `LangGraphAgent({deploymentUrl, graphId})`, plus
   threads, generative UI/A2UI, tool rendering, suggestions, MCP.
...
`ag_ui_langgraph.LangGraphAgent`. The reference serves its graph with **copilotkit's**
`LangGraphAGUIAgent`, whose agent is built with `langchain.agents.create_agent` plus
`CopilotKitMiddleware()` / `StateStreamingMiddleware` — the same construction style our
`create_base_agent` already uses.
...
Browser ──(v2 CopilotKit provider @ root layout)──► /api/copilotkit/[[...slug]]
            ...
            + genUI/A2UI catalog + suggestions          LangGraphAgent({deploymentUrl, graphId})
                                                              │
                                                              ▼
...
2. **v2 runtime + provider skeleton** — bump to `@copilotkit/*` v2 + Next 16; convert
   ...
   the Atlas HTML files, and `styles.css` here.** Wire the frontend `graphId` to the
   backend-derived value via env override (see coupling note below).
...
**2. `copilotkit_server.py` uses copilotkit's agent + derived graphId**
...
- Derive the agent/graph id: `graph_id = get_default_agent() or "dynagent"`. Use it as the
...
- **Log the derived `graph_id` prominently** at startup so step 2 can match the frontend
  route's `graphId`.
...
### graphId / frontend coupling note

The graphId is derived **server-side** from `get_default_agent()`. The v2 frontend route
(step 2) hardcodes/needs a matching `graphId`. Resolution (implemented in step 2, recorded
here so it isn't lost): the frontend route reads `graphId` from an env var
(e.g. `COPILOTKIT_GRAPH_ID`) that the operator sets to the same value the server logs;
default falls back to the configured default agent name. Step 1 only needs to **emit/log**
the value.
