...
def _mock_agent_meta():
    """Isolate create_copilotkit_app from the real agent-config singleton.

    AgentMeta.instance() lazily loads the full roster config for whatever domain
    DYNAGENT_CONFIG_ROOT_DIR points at in this test session; these tests only care
    that the factory reads `.mcp_servers_config`, not that a real domain resolves.
    """
    with patch(
        "autobots_devtools_shared_lib.dynagent.agents.agent_meta.AgentMeta.instance"
    ) as mock_instance:
        mock_instance.return_value.mcp_servers_config = {"weather": {}, "math": {}}
        yield mock_instance
...
@patch(f"{_MODULE}.get_langfuse_handler", return_value=None)
@patch(f"{_MODULE}.create_base_deepagent")
def test_serves_via_rail_agui_agent(mock_create_deep, _mock_lf, mock_graph):
    from autobots_devtools_shared_lib.dynagent.ui.copilotkit_server import create_copilotkit_app
    ...
    with patch(f"{_MODULE}.RailAGUIAgent", wraps=RailAGUIAgent) as spy:
        create_copilotkit_app(agent_name="assistant")
    spy.assert_called_once()
    assert "mcp_servers" in spy.call_args.kwargs
